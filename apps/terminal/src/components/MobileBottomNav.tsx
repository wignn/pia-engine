...
export const MobileBottomNav: React.FC<MobileBottomNavProps> = ({
  ...
  return (
    <nav className="flex md:hidden h-[50px] w-full shrink-0 items-center justify-around border-t border-[#2a2e39] bg-[#1e222d] px-1 z-30 select-none">
      {navItems.map((item) => {
        ...
        return (
          ...
          >
            {item.icon}
            <span className="text-[9px] capitalize tracking-tight">{item.label}</span>
          </button>
        );
      })}
    </nav>
