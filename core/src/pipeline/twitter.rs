...
impl TwitterPipeline {
    ...
    async fn seed(&self, usernames: &str) {
        ...
        info!(
            ...
            "twitter pipeline: seeded (skipped broadcast)"
        );
    }
    ...
    async fn tick(&self, usernames: &str) {
        ...
        for tweet in &tweets {
            let post_data = XPostData {
                ...
                author_avatar: if tweet.author_avatar.is_empty() {
                    None
                } else {
                    Some(tweet.author_avatar.clone())
                },
                created_at: if tweet.created_at.is_empty() {
                    None
                } else {
                    Some(tweet.created_at.clone())
                },
                url: tweet.url.clone(),
                media_urls: if tweet.media_urls.is_empty() {
                    None
                } else {
                    Some(tweet.media_urls.clone())
                },
            };
        ...
        info!(
            total = tweets.len(),
            broadcasted, "twitter pipeline: tick completed"
        );
    }
}
