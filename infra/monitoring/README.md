...
- The first phase monitors host resources, Docker containers, HTTP health checks, and Docker logs.
- `realtime-gateway` exposes app metrics at `/metrics`; Prometheus scrapes it with job `realtime-gateway`.
- The Grafana production overview includes service UP/DOWN tiles, service health history, realtime health latency, active WS connections, WS message rates, fanout/backpressure, and ping/pong rates.
...
curl -fsS http://127.0.0.1:9090/api/v1/targets | grep realtime-gateway
curl -fsS http://127.0.0.1:8020/metrics | grep atlsd_realtime_ws_active_connections
```
