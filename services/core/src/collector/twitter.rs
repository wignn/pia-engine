...
static TWEET_ID_REGEX: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"/status/(\d+)").unwrap());
...
impl TwitterCollector {
    ...
    async fn fetch_user_feed(&self, username: &str) -> Vec<Tweet> {
        ...
        let resp = match self.client.get(&feed_url).send().await {
            Ok(r) => r,
            Err(e) => {
                error!(user = username, error = %e, "twitter: rsshub fetch failed");
                return vec![];
            }
        };
        ...
        let body = match resp.bytes().await {
            Ok(b) => b,
            Err(e) => {
                error!(user = username, error = %e, "twitter: read body failed");
                return vec![];
            }
        };
        ...
        let feed = match feed_rs::parser::parse(&body[..]) {
            Ok(f) => f,
            Err(e) => {
                error!(user = username, error = %e, "twitter: parse RSS failed");
                return vec![];
            }
        };
        ...
        for item in &feed.entries {
            ...
            if newest_id.is_empty() {
                newest_id = tweet_id.clone();
            }
            if !last_seen.is_empty() && tweet_id == last_seen {
                break;
            }
