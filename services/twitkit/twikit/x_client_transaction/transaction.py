...
class ClientTransaction:
    ...
    async def get_indices(self, home_page_response, session, headers):
        ...
        on_demand_match = ON_DEMAND_FILE_REGEX.search(response_str)
        if on_demand_match:
            chunk_index = on_demand_match.group(1)
            hash_match = re.search(
                ON_DEMAND_HASH_PATTERN.format(chunk_index), response_str)
            if hash_match:
        ...
        if not key_byte_indices:
            legacy_match = LEGACY_ON_DEMAND_REGEX.search(response_str)
            if legacy_match:
                on_demand_file_url = f"https://abs.twimg.com/responsive-web/client-web/ondemand.s.{legacy_match.group(1)}a.js"
                try:
                    on_demand_file_response = await session.request(method="GET", url=on_demand_file_url, headers=headers)
                    key_byte_indices_match = INDICES_REGEX.finditer(
                        str(on_demand_file_response.text))
        ...
        if not key_byte_indices:
            key_byte_indices = FALLBACK_INDICES.copy()
